...
const { ChannelRegister, RoleAddRegister, RoleRemoveRegister, TagRegister } = require('../../database/config.json');
...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    const channel = await `${ChannelRegister}`;
    const role = await `${RoleAddRegister}`;
    const roleremove = `${RoleRemoveRegister}`; // buat ngilangin role
    const tag = await `${TagRegister}`; // buat nambahin ganti nambahin contoh diisi [People] nanti namanya jadi [People] Bla bla bla
    ...
    try {
        if (!message.member.permissions.has(Permissions.FLAGS.ADMINISTRATOR)) {
            ...
            if (!message.member.roles.cache.has(`${role}`)){
                ...
                const embed6 = new MessageEmbed().setColor('RED').setTitle('❌ • Error').setDescription(`***Kamu sudah terverifikasi di server ${message.guild.name}!***`);
                return message.reply({ embeds: [embed6] });
            }
        } else {
            const embed5 = new MessageEmbed().setColor('RED').setTitle('❌ • Error').setDescription('Kamu ngga bisa menggunakan command ini karena kamu adalah wibu dek!');
